  ...
  <div class="min-h-[calc(100vh-56px)] p-6 bg-[radial-gradient(circle_at_20%_20%,#f5f7ff,#eef2ff_40%,#e8edf8_80%)]">
    <div class="max-w-4xl mx-auto bg-white rounded-2xl p-8 shadow-[0_22px_80px_rgba(52,64,84,0.14)]">
      ...
      <div class="flex items-center justify-between gap-4 mb-8">
        ...
        <span class="bg-[#dcfce7] text-[#166534] px-4 py-2 rounded-lg font-semibold text-sm" :class="{'bg-red-50 text-red-600': failCount > 0 && successCount === 0, 'bg-yellow-50 text-yellow-600': failCount > 0 && successCount > 0}">
      ...
      <div class="bg-[#fafbff] border border-[#e0e7ff] rounded-xl overflow-hidden mb-8">
          <div v-for="(result, index) in results" :key="index" class="p-4 border-b border-[#e0e7ff] last:border-b-0 flex items-center justify-between hover:bg-white transition-colors">
              <div class="flex items-center gap-3 overflow-hidden">
                  <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0" :class="result.status === 'success' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'">
              ...
              <div class="flex gap-2 shrink-0" v-if="result.status === 'success'">
                  <button @click="openFile(result.outputPath)" class="px-3 py-1.5 rounded-lg text-xs font-semibold text-[#3b82f6] bg-blue-50 hover:bg-blue-100 transition-colors">打开文件</button>
                  <button @click="openFolder(result.outputPath)" class="px-3 py-1.5 rounded-lg text-xs font-semibold text-[#8b5cf6] bg-purple-50 hover:bg-purple-100 transition-colors">文件夹</button>
              </div>
          </div>
