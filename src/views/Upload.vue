...
<template>
  <div class="min-h-[calc(100vh-56px)] p-6 bg-[radial-gradient(circle_at_20%_20%,#f5f7ff,#eef2ff_40%,#e8edf8_80%)]">
    <!-- 下载进度弹窗 -->
    <DownloadProgress :is-visible="isInstalling" :title="installTitle" :detail="installDetail"
      :progress="downloadProgress" :is-error="isError" :error-message="installError" @retry="handleRetry"
      @go-home="handleGoHome" />

    <div
      class="max-w-3xl mx-auto bg-white rounded-2xl p-6 shadow-[0_22px_80px_rgba(52,64,84,0.14)] hover:shadow-[0_26px_90px_rgba(52,64,84,0.18)]"
      :class="{ 'pointer-events-none opacity-40': uploadDisabled }">
      ...
      <div class="inline-flex border border-[#e5e7eb] rounded-xl overflow-hidden mb-4 bg-[#f9fafb]">
        <button class="border-none px-4 py-2 font-semibold bg-transparent text-[#4b5563] cursor-pointer transition-all"
          :class="{ 'bg-[linear-gradient(90deg,#6366f1,#8b5cf6)] text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.08)]': activeTab === 'file' }"
          @click="switchTab('file')">单文件上传</button>
        <button class="border-none px-4 py-2 font-semibold bg-transparent text-[#4b5563] cursor-pointer transition-all"
          :class="{ 'bg-[linear-gradient(90deg,#6366f1,#8b5cf6)] text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.08)]': activeTab === 'batch' }"
          @click="switchTab('batch')">批量上传</button>
        <button class="border-none px-4 py-2 font-semibold bg-transparent text-[#4b5563] cursor-pointer transition-all"
          :class="{ 'bg-[linear-gradient(90deg,#6366f1,#8b5cf6)] text-white shadow-[inset_0_0_0_1px_rgba(255,255,255,0.08)]': activeTab === 'text' }"
          @click="switchTab('text')">markdown 文本</button>
      </div>

      <!-- Single File Tab -->
      <div v-if="activeTab === 'file'"
        class="border border-dashed border-[#e5e7eb] rounded-xl p-4 bg-[#f8fafc] flex flex-col min-h-61">
        <div
        ...
        <div v-if="selectedFile"
          class="mt-1 flex flex-1 items-center justify-between gap-2 px-3 py-2 rounded-lg bg-[#eef2ff] text-[#1f2937]">
          <span class="truncate text-m font-semibold">{{ selectedFile.name }}</span>
          <button
            class="border-none bg-[#e0e7ff] text-[#312e81] rounded-full w-5 h-5 cursor-pointer font-bold text-xs flex items-center justify-center shrink-0"
            @click.stop="clearSelection">✕</button>
        </div>
      </div>
      ...
      <div v-else-if="activeTab === 'batch'"
        ...
        <div class="flex-1 overflow-y-auto max-h-48 space-y-2 pr-1">
          ...
          <div v-for="file in uploadStore.files" :key="file.name"
            class="flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-white border border-gray-100 shadow-sm">
            <span class="truncate text-sm font-medium text-gray-700">{{ file.name }}</span>
            <button
              class="border-none bg-red-50 text-red-500 hover:bg-red-100 rounded-full w-5 h-5 cursor-pointer font-bold text-xs flex items-center justify-center shrink-0 transition-colors"
              @click.stop="removeBatchFile(file.name)">✕</button>
          </div>
        </div>
      </div>
      ...
      <div v-else class="border border-dashed border-[#e5e7eb] rounded-xl px-4 py-3 bg-[#f8fafc]">
        ...
        <textarea v-model="textContent"
          class="w-full border border-[#e5e7eb] rounded-xl p-3.5 text-sm font-mono bg-white text-[#111827] resize-y min-h-45"
          placeholder="粘贴你的 Markdown，图片引用会被自动扫描并复制到临时目录。" rows="7"></textarea>
      </div>

      <div v-if="prepareError" class="mt-3 px-3.5 py-3 rounded-xl bg-[#fef2f2] text-[#b91c1c] border border-[#fecdd3]">
        {{ prepareError }}
      </div>

      <div class="flex justify-end mt-5">
        <button
          class="bg-[linear-gradient(90deg,#22c55e,#16a34a)] text-white px-7 py-3 rounded-xl text-base font-bold cursor-pointer transition-all shadow-[0_12px_30px_rgba(34,197,94,0.25)] hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(34,197,94,0.3)] disabled:opacity-60 disabled:cursor-not-allowed disabled:shadow-none"
          :disabled="uploadDisabled || isPreparing" @click="nextStep">
  ...
  <div v-if="showMdSelectionDialog" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-6 shadow-[0_25px_50px_rgba(0,0,0,0.25)] max-w-sm w-11/12">
      ...
      <div class="mb-6 border border-[#e5e7eb] rounded-xl overflow-hidden bg-[#f9fafb] max-h-64 overflow-y-auto">
        <div v-for="(mdFile, idx) in pendingMarkdownFiles" :key="idx"
          class="p-3 border-b border-[#e5e7eb] last:border-b-0 cursor-pointer hover:bg-[#f3f4f6] transition-all"
          :class="{ 'bg-[#eef2ff] border-l-4 border-l-[#6366f1]': selectedMdFile === mdFile }"
          @click="selectedMdFile = mdFile">
      ...
      <div class="flex gap-3 justify-end">
        <button
          class="px-5 py-2 rounded-lg text-sm font-semibold text-[#6b7280] bg-[#f3f4f6] cursor-pointer transition-all hover:bg-[#e5e7eb]"
          @click="showMdSelectionDialog = false">取消</button>
        <button
          class="px-5 py-2 rounded-lg text-sm font-semibold text-white bg-[linear-gradient(90deg,#22c55e,#16a34a)] cursor-pointer transition-all shadow-[0_12px_30px_rgba(34,197,94,0.25)] hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(34,197,94,0.3)]"
          @click="confirmMdSelection">确认</button>
      </div>
    </div>
  </div>
</template>
