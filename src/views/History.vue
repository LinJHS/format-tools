    ...
    <div v-else class="space-y-4">
      <div v-for="record in displayRecords" :key="record.id"
        ...
        <div class="flex-1">
          <div class="flex items-center gap-2 mb-1">
            ...
            <span v-if="record.status === 'success'"
              class="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded">成功</span>
        ...
        <div class="flex gap-2 ml-4">
          <button v-if="record.status === 'success' && record.outputPath" @click="openFile(record.outputPath)"
            class="bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1.5 rounded text-sm hover:bg-blue-100">
            打开文件
          </button>
          <button v-if="record.status === 'success' && record.outputPath" @click="openFolder(record.outputPath)"
